...
for s in seed:
    ...
    # Evaluate different splits for current seed
    for train_split in splits:
        print(f"Seed: {s}, Train split: {train_split}")
        ...
        results_per_seed[s][train_split] = {
            'MAE': mseList,
            'Miscoverage': coverageList,
            'Area': areaList
        }

    # Evaluate single learner (split = 1.0) for current seed
    print(f"Seed: {s}, Train split: 1.0 (single learner)")
    train_data, test_data_UD, UD_A, UD_B = splitData(Learner_A, Learner_B, train_split=1.0, SEED=s)
    model = torch.load(f'Experiment_Single_{learner}_{s}.pth', weights_only=False)
    model.eval()
    mseList, coverageList, areaList = evaluate_all(UD_A, model)

    results_per_seed[s][1.0] = {
        'MAE': mseList,
        'Miscoverage': coverageList,
        'Area': areaList
    }

# Perform statistical comparisons per seed
all_splits = [1.0, 0.8, 0.6, 0.4, 0.2]

metrics = ['MAE', 'Miscoverage', 'Area']
results_dict = {}
print(f"Mixed training for learners {LEARNER_ZOO[Learner_A]} and {LEARNER_ZOO[Learner_B]} evaluated on Unseen Data for {LEARNER_ZOO[learner]}.")
for s in seed:
    print(f"\n{'=' * 50}")
    print(f"STATISTICAL ANALYSIS FOR SEED {s}")
    print(f"{'=' * 50}")
    ...
    for metric_name in metrics:
        ...
        for split1, split2 in itertools.combinations(all_splits, 2):
            data1 = results_per_seed[s][split1][metric_name]
            data2 = results_per_seed[s][split2][metric_name]

            # Compare both directions
            stat_less, p_less = stats.wilcoxon(data1, data2, alternative='less')
            stat_greater, p_greater = stats.wilcoxon(data1, data2, alternative='greater')

            # Pick the more significant test
            if p_less < p_greater and p_less < 0.05:
                p_values.append(p_less)
                directions.append("less")
                comparisons.append((split1, split2, stat_less))
            elif p_greater < p_less and p_greater < 0.05:
                p_values.append(p_greater)
                directions.append("greater")
                comparisons.append((split1, split2, stat_greater))
            else:
                p_values.append(max(p_less, p_greater))
                directions.append("not significant")
                comparisons.append((split1, split2, min(stat_less, stat_greater)))

        _, corrected_pvals, _, _ = smm.multipletests(p_values, method='bonferroni')

        results_dict[s][metric_name] = {}  # Store results per metric

        for i, ((split1, split2, stat), raw_p, corr_p, direction) in enumerate(
            zip(comparisons, p_values, corrected_pvals, directions)
        ):
            if corr_p < 0.05:
                print(
                    f"Split {split1} vs {split2}:(*) {direction}"
                )
                key = f"{split1} vs {split2}"
                results_dict[s][metric_name][key] = (corr_p, direction)
            else:
                print(
                    f"Split {split1} vs {split2}: {direction}"
                )
                key = f"{split1} vs {split2}"
                results_dict[s][metric_name][key] = (corr_p, "not significant")
